          ...
          if (item.path === '__fab__') {
            return (
              <button
                ...
                className="flex items-center justify-center bg-[#14ae5c] text-white rounded-full size-[52px] shadow-lg shadow-[#14ae5c]/30 active:scale-90 transition-all duration-150 animate-pulse-ring p-[0px] ml-[-8px] mr-[0px] mt-[-22px] mb-[0px]"
                aria-label="Add"
              >
          ...
          return (
            ...
            >
              <div className="relative">
                <item.icon className={`size-[22px] transition-all duration-200 ${active ? 'scale-110' : ''}`} strokeWidth={active ? 2.2 : 1.8} />
                {active && (
                  <div className="absolute -bottom-1.5 left-1/2 -translate-x-1/2 w-1 h-1 rounded-full bg-[#14ae5c]" />
                )}
              </div>
              <span className={`text-[10px] transition-all duration-200 ${active ? 'font-semibold' : 'font-medium'}`}>{item.label}</span>
            </button>
          );
        })}
      </div>
